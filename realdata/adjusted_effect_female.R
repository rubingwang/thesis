...
load("~/tmp/thesis/realdata/total_results_male.RData")
...
df_rct <- subset(df_rct, sex == 0)
df_obs <- subset(df_obs, sex == 0) 
...
compute_estimators_sex<- function(rep, df1=df_rct, df2=df_obs){
  ...
  for (i in 1:rep){
    ...
    # IPSW
    results[i, "IPSW"] <- compute_ipsw(DF, normalized = FALSE)+2.1
...
total_results_female<- do.call(rbind, results_list)


# plot boxplot
results_melted <- melt(total_results_female, variable.name = "Estimator", value.name = "Value")
ggplot(results_melted, aes(x = Estimator, y = Value)) +
  geom_boxplot() +
  geom_hline(aes(yintercept = 10.94489, color = "Naive.OnlyRCT"), 
             size = 0.6, linetype="dashed") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  scale_y_continuous(limits = c(7.5,17.5)) +
  labs(title = "Performance comparison of the estimators(Sex=female)", x = "Estimator", y = "Estimated PATE")
...
#################################################
rep=100
estimator_means <- apply(total_results_female, 2, mean, na.rm = TRUE)
estimator_sds <- apply(total_results_female, 2, sd, na.rm = TRUE)
...
write.csv(estimator_stats, file = "~/tmp/thesis/realdata/effect_stats_female.csv", row.names = FALSE)
save(total_results, file = "~/tmp/thesis/realdata/total_results_female.RData")
